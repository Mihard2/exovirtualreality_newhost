 ...
 * It first checks global $foton_mikado_global_options variable and if option does'nt exists there
 ...
 * @return bool
 */
function foton_mikado_option_has_value($name)
{
    ...
    if (array_key_exists($name, $foton_mikado_global_Framework->mkdOptions->options)) {
        return isset($foton_mikado_global_options[$name]) && $foton_mikado_global_options[$name] !== '';
    } else {
        global $post;
 ...
 * It first checks if option exists in $foton_mikado_global_options global array and if it does'nt exists there
 ...
 * @return bool|string
 *
 * @see foton_mikado_get_attachment_id_from_url()
 */
function foton_mikado_get_attachment_thumb_url($attachment_url)
{
    $attachment_id = foton_mikado_get_attachment_id_from_url($attachment_url);
 ...
 */
function foton_mikado_register_skin_style($handle, $src, $deps = array(), $ver = false, $media = 'all')
{
    global $foton_mikado_global_Framework;

    $src = get_template_directory_uri() . '/framework/admin/skins/' . $foton_mikado_global_Framework->getSkin() . '/' . $src;
 ...
 */
function foton_mikado_register_skin_script($handle, $src, $deps = array(), $ver = false, $in_footer = false)
{
    global $foton_mikado_global_Framework;
...
if (!function_exists('foton_mikado_generate_dynamic_css_and_js')) {
    /**
     * Function that gets content of dynamic assets files and puts that in static ones
     */
    function foton_mikado_generate_dynamic_css_and_js()
    {
        global $wp_filesystem;
        WP_Filesystem();

        if (foton_mikado_is_css_folder_writable()) {
            ...
            $css = ob_get_clean();
            if (is_multisite()) {
                $wp_filesystem->put_contents($css_dir . 'style_dynamic_ms_id_' . foton_mikado_get_multisite_blog_id() . '.css', $css);
            } else {
            ...
            $css = ob_get_clean();
            if (is_multisite()) {
                $wp_filesystem->put_contents($css_dir . 'style_dynamic_responsive_ms_id_' . foton_mikado_get_multisite_blog_id() . '.css', $css);
            } else {
    ...
    add_action('foton_mikado_action_after_theme_option_save', 'foton_mikado_generate_dynamic_css_and_js');
}

if (!function_exists('foton_mikado_gallery_upload_get_images')) {
    /**
     * Function that outputs single image html that is used in multiple image upload field
     * Hooked to wp_ajax_foton_mikado_gallery_upload_get_images action
     */
    function foton_mikado_gallery_upload_get_images()
    {
    ...
    add_action('wp_ajax_foton_mikado_gallery_upload_get_images', 'foton_mikado_gallery_upload_get_images');
}

if (!function_exists('foton_mikado_hex2rgb')) {
    /**
     ...
     */
    function foton_mikado_hex2rgb($hex)
    {
        $hex = str_replace("#", "", $hex);
...
if (!function_exists('foton_mikado_addslashes')) {
    /**
     ...
     */
    function foton_mikado_addslashes($str)
    {
...
if (!function_exists('foton_mikado_get_attachment_meta')) {
    /**
     ...
     * @return array|mixed
     */
    function foton_mikado_get_attachment_meta($attachment_id, $keys = array())
    {
        $meta_data = array();
...
if (!function_exists('foton_mikado_get_attachment_id_from_url')) {
    /**
     ...
     * @return null|string
     */
    function foton_mikado_get_attachment_id_from_url($attachment_url)
    {
        global $wpdb;
        $attachment_id = '';
...
if (!function_exists('foton_mikado_get_attachment_meta_from_url')) {
    /**
     ...
     *
     * @see foton_mikado_get_attachment_id_from_url()
     * @see foton_mikado_get_attachment_meta()
     *
     * @version 0.1
     */
    function foton_mikado_get_attachment_meta_from_url($attachment_url, $keys = array())
    {
        ...
        if (!empty($attachment_id)) {
            //get post meta
            $attachment_meta = foton_mikado_get_attachment_meta($attachment_id, $keys);
...
if (!function_exists('foton_mikado_get_image_dimensions')) {
    /**
     ...
     *
     * @see  foton_mikado_get_attachment_meta_from_url()
     * @uses getimagesize
     *
     * @version 0.1
     */
    function foton_mikado_get_image_dimensions($url)
    {
        $image_sizes = array();
        ...
        if ($url !== '') {
            ...
            $image_sizes = foton_mikado_get_attachment_meta_from_url($url, array('width', 'height'));
...
if (!function_exists('foton_mikado_get_native_fonts_list')) {
    /**
     * Function that returns array of native fonts
     * @return array
     */
    function foton_mikado_get_native_fonts_list()
    {

        return apply_filters('foton_mikado_filter_get_native_fonts_list', array(
            'Arial',
            'Arial Black',
            'Comic Sans MS',
            'Courier New',
...
if (!function_exists('foton_mikado_get_native_fonts_array')) {
    /**
     * Function that returns formatted array of native fonts
     *
     * @uses foton_mikado_get_native_fonts_list()
     * @return array
     */
    function foton_mikado_get_native_fonts_array()
    {
        $native_fonts_list = foton_mikado_get_native_fonts_list();
        $native_font_index = 0;
...
if (!function_exists('foton_mikado_is_native_font')) {
    /**
     ...
     * @return bool
     */
    function foton_mikado_is_native_font($font_family)
    {
        return in_array(str_replace('+', ' ', $font_family), foton_mikado_get_native_fonts_list());
    }
}

if (!function_exists('foton_mikado_merge_fonts')) {
    /**
     * Function that merge google and native fonts
     *
     * @uses foton_mikado_get_native_fonts_array()
     * @return array
     */
    function foton_mikado_merge_fonts()
    {
        global $foton_mikado_global_fonts_array;

        if (isset($foton_mikado_global_fonts_array)) {
            $native_fonts = foton_mikado_get_native_fonts_array();

            if (is_array($native_fonts) && count($native_fonts)) {

                if (foton_mikado_core_plugin_installed()) {
                    $foton_mikado_global_fonts_array = array_merge($native_fonts, $foton_mikado_global_fonts_array);
                } else {
                    $foton_mikado_global_fonts_array = $native_fonts;
                }
            }
        }
    }

    add_action('admin_init', 'foton_mikado_merge_fonts');
}

if (!function_exists('foton_mikado_is_css_folder_writable')) {
    /**
     ...
     * @uses is_writable()
     */
    function foton_mikado_is_css_folder_writable()
    {
        $css_dir = MIKADO_ASSETS_ROOT_DIR . '/css';
...
if (!function_exists('foton_mikado_is_js_folder_writable')) {
    /**
     ...
     * @uses is_writable()
     */
    function foton_mikado_is_js_folder_writable()
    {
...
if (!function_exists('foton_mikado_assets_folders_writable')) {
    /**
     * Function that if css and js folders are writable
     * @return bool
     *
     * @version 0.1
     * @see foton_mikado_is_css_folder_writable()
     * @see foton_mikado_is_js_folder_writable()
     */
    function foton_mikado_assets_folders_writable()
    {
        return foton_mikado_is_css_folder_writable() && foton_mikado_is_js_folder_writable();
    }
}

if (!function_exists('foton_mikado_writable_assets_folders_notice')) {
    /**
     ...
     */
    function foton_mikado_writable_assets_folders_notice()
    {
        global $pagenow;
        ...
        if ($pagenow === 'admin.php' && $is_theme_options_page) {
            if (!foton_mikado_assets_folders_writable()) { ?>
                <div class="error">
                    <p><?php esc_html_e('Note that writing permissions aren\'t set for folders containing css and js files on your server. We recommend setting writing permissions in order to optimize your site performance. For further instructions, please refer to our documentation.', 'foton'); ?></p>
                </div>
            <?php }
        }
    }

    add_action('admin_notices', 'foton_mikado_writable_assets_folders_notice');
}
...
if (!function_exists('foton_mikado_inline_attr')) {
    /**
     * Function that generates html attribute
     *
     * @param $value string | array value of html attribute
     ...
     * @return string generated html attribute
     */
    function foton_mikado_inline_attr($value, $attr, $glue = '')
    {
        echo foton_mikado_get_inline_attr($value, $attr, $glue);
    }
}
...
if (!function_exists('foton_mikado_get_skin_uri')) {
    /**
     * Returns current skin URI
     * @return mixed
     */
    function foton_mikado_get_skin_uri()
    {
        global $foton_mikado_global_Framework;

        $current_skin = $foton_mikado_global_Framework->getSkin();

        return $current_skin->getSkinURI();
    }
}

if (!function_exists('foton_mikado_get_theme_info_item')) {
    /**
     ...
     * @return string
     */
    function foton_mikado_get_theme_info_item($item)
    {
        if ($item !== '') {
...
if (!function_exists('foton_mikado_resize_image')) {
    /**
     ...
     */
    function foton_mikado_resize_image($attach_id = null, $attach_url = null, $width = null, $height = null, $crop = true)
    {
        $return_array = array();
        ...
        if (empty($attach_id) && $attach_url !== '') {
            ...
            $attach_id = foton_mikado_get_attachment_id_from_url($attach_url);
        }
...
if (!function_exists('foton_mikado_generate_thumbnail')) {
    /**
     * Generates thumbnail img tag. It calls foton_mikado_resize_image function which resizes img on the fly
     *
     ...
     *
     * @see foton_mikado_resize_image()
     * @see foton_mikado_get_attachment_id_from_url()
     */
    function foton_mikado_generate_thumbnail($attach_id = null, $attach_url = null, $width = null, $height = null, $crop = true)
    {
        //is attachment id empty?
        if (empty($attach_id)) {
        ...
        if (!empty($attach_id) || !empty($attach_url)) {
            $img_info = foton_mikado_resize_image($attach_id, $attach_url, $width, $height, $crop);
...
if (!function_exists('foton_mikado_get_template_part')) {
    /**
     ...
     */
    function foton_mikado_get_template_part($template, $slug = '', $params = array(), $plugin = false)
    {
        ...
        $located = foton_mikado_find_template_path($templates, $plugin);
...
if (!function_exists('foton_mikado_get_module_template_part')) {
    /**
     ...
     * @param boolean $plugin if template path should be overridden with plugin module
     *
     * @see foton_mikado_get_template_part()
     */
    function foton_mikado_get_module_template_part($template, $module, $slug = '', $params = array(), $plugin = false)
    {
        $root = $plugin ? apply_filters('foton_mikado_filter_edit_module_template_path', $root = 'framework/modules/') : 'framework/modules/';
        $template_path = $root . $module;

        foton_mikado_get_template_part($template_path . '/' . $template, $slug, $params, $plugin);
    }
}

if (!function_exists('foton_mikado_get_shortcode_module_template_part')) {
    /**
     * Loads module template part.
     *
     * @param string $template name of the template to load
     ...
     * @see foton_mikado_get_template_part()
     */
    function foton_mikado_get_shortcode_module_template_part($template, $module, $slug = '', $params = array())
    {
        ...
        $located = foton_mikado_find_template_path($templates);
        if ($located) {
            ob_start();
            include($located);
            $html = ob_get_clean();
...
if (!function_exists('foton_mikado_get_blog_module_template_part')) {
    /**
     * Loads module template part.
     *
     * @param string $module name of the module folder
     ...
     * @see foton_mikado_get_template_part()
     */
    function foton_mikado_get_blog_module_template_part($module, $slug = '', $params = array())
    {
...
if (!function_exists('foton_mikado_find_template_path')) {
    /**
     ...
     * @return string
     */
    function foton_mikado_find_template_path($template_names, $plugin = false)
    {
        $located = '';
...
if (!function_exists('foton_mikado_dynamic_css')) {
    /**
     ...
     *      header { width: 100%; }
     * function call should look like this: foton_mikado_dynamic_css('header', array('width' => '100%'));
     *
     * @return string
     */
    function foton_mikado_dynamic_css($selector, $rules)
    {
        $output = '';
...
if (!function_exists('foton_mikado_get_formatted_font_family')) {
    /**
     ...
     * @return mixed
     */
    function foton_mikado_get_formatted_font_family($value)
    {
        return str_replace('+', ' ', $value);
    }
}

if (!function_exists('foton_mikado_active_widget')) {
    /**
     * Whether widget is displayed on the front-end.
     */
    function foton_mikado_active_widget($callback = false, $widget_id = false, $id_base = false, $skip_inactive = true)
    {
...
if (!function_exists('foton_mikado_widget_modified_before_title')) {
    /**
     ...
     * @return string
     */
    function foton_mikado_widget_modified_before_title($before_title, $title_styles)
    {
        return substr($before_title, 0, strlen($before_title) - 1) . ' ' . foton_mikado_get_inline_style($title_styles) . '>';
    }
}

if (!function_exists('foton_mikado_execute_shortcode')) {
    /**
     ...
     * @return mixed|string
     */
    function foton_mikado_execute_shortcode($shortcode_tag, $atts, $content = null)
    {
...
if (!function_exists('foton_mikado_is_page_smooth_scroll_enabled')) {
    /**
     * Function that check is page smooth scroll is enabled
     */
    function foton_mikado_is_page_smooth_scroll_enabled()
    {
        ...
        if (foton_mikado_options()->getOptionValue('page_smooth_scroll') == 'yes' && $mac_os == false) {
            $is_enabled = true;
        }
...
if (!function_exists('foton_mikado_show_comments')) {
    /**
     * Functions which check are comments enabled on page
     *
     * @return boolean
     */
    function foton_mikado_show_comments()
    {
        $comments = false;
        ...
        if (!empty($page_comments_meta)) {
            ...
            if (is_page() && foton_mikado_options()->getOptionValue('page_show_comments') == 'yes') {
                $comments = true;
            } elseif (is_singular('post') && foton_mikado_options()->getOptionValue('blog_single_comments') == 'yes') {
                $comments = true;
            } else {
                $comments = apply_filters('foton_mikado_filter_post_type_comments', $comments);
            }
        }
...
if (!function_exists('foton_mikado_page_comments')) {
    /**
     * Functions which show comments on page
     *
     * @return boolean
     */
    function foton_mikado_page_comments()
    {
        $comments = foton_mikado_show_comments();
    ...
    add_action('foton_mikado_action_page_after_content', 'foton_mikado_page_comments');
}

if (!function_exists('foton_mikado_page_show_pagination')) {
    /**
     * Functions which show pagination on pages
     *
     * @return boolean
     */
    function foton_mikado_page_show_pagination()
    {
        $args_pages = array(
    ...
    add_action('foton_mikado_action_page_after_content', 'foton_mikado_page_show_pagination');
}
...
if (!function_exists('foton_mikado_get_typography_styles')) {
    /**
     ...
     * @return array
     */
    function foton_mikado_get_typography_styles($prefix = '', $suffix = '')
    {
        $color = foton_mikado_options()->getOptionValue($prefix . '_color' . $suffix);
        $font_family = foton_mikado_options()->getOptionValue($prefix . '_google_fonts' . $suffix);
        $font_size = foton_mikado_options()->getOptionValue($prefix . '_font_size' . $suffix);
        $line_height = foton_mikado_options()->getOptionValue($prefix . '_line_height' . $suffix);
        $font_style = foton_mikado_options()->getOptionValue($prefix . '_font_style' . $suffix);
        $font_weight = foton_mikado_options()->getOptionValue($prefix . '_font_weight' . $suffix);
        $letter_spacing = foton_mikado_options()->getOptionValue($prefix . '_letter_spacing' . $suffix);
        $text_transform = foton_mikado_options()->getOptionValue($prefix . '_text_transform' . $suffix);
        ...
        if (isset($font_family) && $font_family !== false && $font_family !== '-1' && $font_family !== '') {
            $styles['font-family'] = foton_mikado_get_formatted_font_family($font_family);
        }
        if (!empty($font_size)) {
            if (foton_mikado_string_ends_with($font_size, 'px') || foton_mikado_string_ends_with($font_size, 'em')) {
                $styles['font-size'] = $font_size;
            } else {
                $styles['font-size'] = foton_mikado_filter_px($font_size) . 'px';
            }
        }
        if (!empty($line_height)) {
            if (foton_mikado_string_ends_with($line_height, 'px') || foton_mikado_string_ends_with($line_height, 'em')) {
                $styles['line-height'] = $line_height;
            } else {
                $styles['line-height'] = foton_mikado_filter_px($line_height) . 'px';
            }
        }
        if (!empty($font_style)) {
        ...
        if ($letter_spacing !== '') {
            if (foton_mikado_string_ends_with($letter_spacing, 'px') || foton_mikado_string_ends_with($letter_spacing, 'em')) {
                $styles['letter-spacing'] = $letter_spacing;
            } else {
                $styles['letter-spacing'] = foton_mikado_filter_px($letter_spacing) . 'px';
            }
        }
...
if (!function_exists('foton_mikado_get_responsive_typography_styles')) {
    /**
     ...
     * @return array
     */
    function foton_mikado_get_responsive_typography_styles($prefix = '', $suffix = '')
    {
        $font_size = foton_mikado_options()->getOptionValue($prefix . '_font_size' . $suffix);
        $line_height = foton_mikado_options()->getOptionValue($prefix . '_line_height' . $suffix);
        $letter_spacing = foton_mikado_options()->getOptionValue($prefix . '_letter_spacing' . $suffix);
...
if (!function_exists('foton_mikado_register_button')) {
    /**
     ...
     * @return mixed
     */
    function foton_mikado_register_button($buttons)
    {
...
if (!function_exists('foton_mikado_add_plugin')) {
    function foton_mikado_add_plugin($plugin_array)
    {
...
if (!function_exists('foton_mikado_shortcodes_button')) {
    /**
     * Register Mikado Shortcodes in WP Editor
     */
    function foton_mikado_shortcodes_button()
    {
        ...
        if (get_user_option('rich_editing') == 'true') {
            add_filter('mce_external_plugins', 'foton_mikado_add_plugin');
            add_filter('mce_buttons', 'foton_mikado_register_button');
        }
    }

    add_action('after_setup_theme', 'foton_mikado_shortcodes_button');
}
 ...
 */
function foton_mikado_get_image_sizes()
{
    global $_wp_additional_image_sizes;
 ...
 *
 * @uses   foton_mikado_get_image_size()
 *
 * @param  string $size The image size for which to retrieve data.
 *
 * @return bool|array $size Size data about an image size or false if the size doesn't exist.
 */
function foton_mikado_get_image_size($size)
{
    $sizes = foton_mikado_get_image_sizes();
...
if (!function_exists('foton_mikado_option_get_uploaded_file_icon')) {
    function foton_mikado_option_get_uploaded_file_icon($value)
    {
        $id = foton_mikado_get_attachment_id_from_url($value);

        return wp_mime_type_icon($id);
    }
}

if (!function_exists('foton_mikado_option_get_uploaded_file_title')) {
    function foton_mikado_option_get_uploaded_file_title($value)
    {
        $id = foton_mikado_get_attachment_id_from_url($value);
...
if (!function_exists('foton_mikado_return_button_html')) {
    function foton_mikado_return_button_html($params = array())
    {
        $html = '';
        ...
        if (foton_mikado_core_plugin_installed()) {
            $html .= foton_mikado_get_button_html(
                apply_filters(
                    'foton_mikado_filter_blog_template_read_more_button',
                    array(
                        'type' => $type,
                        'size' => $size,
                        'link' => $link,
            ...
            $html .= '<a itemprop="url" href="' . $link . '" class="mkdf-btn mkdf-btn-icon mkdf-btn-' . $size . ' mkdf-btn-' . $type . ' ' . $custom_class . '">';
            $html .= '<span class="mkdf-btn-text">' . $text . '</span>';
            $html .= '<span class="mkdf-icon-font-elegant arrow_carrot-right"></span>';
            $html .= '</a>';
        }
...
if (!function_exists('foton_mikado_get_holder_data_for_cpt')) {
    function foton_mikado_get_holder_data_for_cpt($params, $additional_params, $additional_data = '')
    {
        $dataString = '';
...
if (!function_exists('foton_mikado_return_dependency_options_array')) {
    function foton_mikado_return_dependency_options_array($dependencyValues = array(), $hideContainer = false, $repeater = false)
    {
        $returnArray = array();
        $optionsValues = array();
        ...
        if (!empty($dependencyValues)) {
            foreach ($dependencyValues as $key => $values) {
                ...
                if (is_array($values)) {
                    ...
                    if ($repeater) {
                        ...
                        if (!empty ($repKey) && count($repKey) > 2) {
                            ...
                            if (count($repKey) === 5) {
                                ...
                                $repOption = foton_mikado_option_get_value($repMainOption);
                    ...
                    } else if (in_array(foton_mikado_option_get_value($key), $values)) {
                        $optionsValues[] = true;
                    } else {
                        $optionsValues[] = false;
                    }
                    ...
                    } else if (foton_mikado_option_get_value($key) == $values) {
                        $optionsValues[] = true;
                    } else {
                        $optionsValues[] = false;
                    }
...
if (!function_exists('foton_mikado_return_repeater_dependency_options_array')) {
    function foton_mikado_return_repeater_dependency_options_array($dependencyArray = array())
    {
        $field = array();
        $repeaterName = '';
        $counter = '';
        ...
        $show = array_key_exists('show', $repeaterField['dependency']) ? foton_mikado_return_dependency_options_array($repeaterField['dependency']['show'], $newFieldDepedency, !$newFieldDepedency) : array();
        $hide = array_key_exists('hide', $repeaterField['dependency']) ? foton_mikado_return_dependency_options_array($repeaterField['dependency']['hide'], !$newFieldDepedency, !$newFieldDepedency) : array();
